...
int main(int argc, char *argv[]) {
  ...
  while(1){
    ...
    char recvbuf[65536];
    bzero(recvbuf, 65536);
    int nrec = recvfrom(sockfd, recvbuf, 65536, 0, (struct sockaddr*)&clientaddr, &addrLen);
    if (nrec < 0) {
    ...
    int offset = sizeof(struct DNS_HEADER) + domainLen + 1 + sizeof(struct QUESTION);
    ...
    // 发送DNS响应报文
    int nsent = sendto(sockfd, recvbuf, offset, 0, (struct sockaddr *)&clientaddr, addrLen);
    if (nsent < 0) {
        perror("sendto");
        continue;
    }
