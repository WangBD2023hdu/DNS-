...
size_t send_query(int sockfd, const char *domain, sockaddr_in * servaddr){
  ...
  char *hostname_dup = strdup(domain); // strdup --> malloc
    ...
    *(netdomian + offset) = len;
    offset++;
    strncpy(netdomian+offset, token, len+1);//len+1是由于qname的最后必须以0结尾
    offset += len;
  ...
  memcpy(buf + sizeof(header) + strlen(netdomian) + 1, &question, sizeof(question));
  size_t buflen = sizeof(header) + strlen(netdomian) + 1 + sizeof(question);
  if(sendto(sockfd, buf, sizeof(header) + strlen(netdomian) + 1 + sizeof(question), 0, (sockaddr *)servaddr, sizeof(sockaddr_in))<0){
    perror("send failure!");
    return -1;
  }
...
int main(int argc, char *argv[]) {
  ...
  recvfrom(sockfd, (char *)revbuf, 65535, 0, (struct sockaddr *)&servaddr, (socklen_t*)&len);
  printf("recv response!\n");
  struct DNS_HEADER *respondheader;
  respondheader = (struct DNS_HEADER *) revbuf;
  printf("id %u qu count %d ans cout %u\n:", ntohs(respondheader->id), ntohs(respondheader->q_count), ntohs(respondheader->ans_count));
  char * readbuf = revbuf + sendlen;

  struct RES_RECORD answer[30];
  answer[0].resource = (struct R_DATA *)(readbuf + 2);
  answer[0].rdata = (unsigned char *)malloc(ntohs(answer[0].resource->data_len)+1);
  
  struct sockaddr_in addres;
  if(ntohs(answer[0].resource->type) == T_A){
    printf("ttl:%d\n", ntohs(answer[0].resource->ttl));
    memcpy(answer[0].rdata, (revbuf + sendlen + 2 + sizeof(struct R_DATA)), ntohs(answer[0].resource->data_len));
